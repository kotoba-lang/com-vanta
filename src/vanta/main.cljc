(ns vanta.main
  "Kotodama WASM entrypoint for the Vanta clean-room actor (L4) — Clojure port.
  ...
  facts (`vanta.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"vanta.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "vanta-compat" :tier tier :entities entities} 200])
